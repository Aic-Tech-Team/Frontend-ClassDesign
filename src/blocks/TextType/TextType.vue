...
const executeTypingAnimation = () => {
  ...
  if (isDeleting.value) {
    if (displayedText.value === "") {
      ...
      const sentence = textArray.value[currentTextIndex.value];
      if (sentence) {
        props.onSentenceComplete?.(sentence, currentTextIndex.value);
      }
    ...
    if (processedText && currentCharIndex.value < processedText.length) {
      timeout = setTimeout(
        () => {
  ...
  >
    ...
    <span
      ...
      :class="`ml-1 inline-block opacity-100 ${
        ...
        (textArray[currentTextIndex] && (currentCharIndex < (textArray[currentTextIndex]?.length ?? 0) || isDeleting))
          ? 'hidden'
          : ''
      } ${cursorClassName}`"
    >
